    ...
    return (
      <>
        ...
        {this.state.loadProgress < 1 && (
          <mesh
            name="load-mesh"
            rotation={this.props.progressRotation}
            position={this.props.center}
          >
            <box
              key="progress"
              name="boxProgress"
              height={this.props.scaleTo / 15}
              width={this.props.scaleTo}
              depth={this.props.scaleTo / 30}
              scaling={new Vector3(this.state.loadProgress, 1, 1)}
              position={
                new Vector3(this.props.scaleTo / 2, 0, this.props.scaleTo / 60)
              }
              setPivotMatrix={[Matrix.Translation(-this.props.scaleTo, 0, 0)]}
              setPreTransformMatrix={[
                Matrix.Translation(-this.props.scaleTo / 2, 0, 0),
              ]}
            >
              <standardMaterial
                name="progress-mat"
                diffuseColor={this.props.progressBarColor}
